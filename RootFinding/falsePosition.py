...
def falsePositionMethod(func, x0, x1, epsilon, nMax= 100):
# initialization
	i = 0
	x2 = 0
...
#======================================= plot false position Method
root, iterX, iterY, iterCount = falsePositionMethod( f, -5, -2, 0.01 )
fig, ax = plt.subplots()
ax.set_xlabel('x')
ax.axhline(0)
ax.plot(x, y)
ax.scatter( x= iterX, y= iterY )
ax.set_title( 'false position Method: {} iterations'.format( len(iterCount) ) )
